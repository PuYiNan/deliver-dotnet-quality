...
import { installSkill, legacySkillNames, restoreSkill, skillName } from './installer.mjs';
  ...
  if (command === 'doctor') {
    ...
    const agentPaths = Object.fromEntries(supportedAgents.map((agent) => {
      ...
      return [agent, {
        ...
        installed: fs.existsSync(path.join(root, skillName, 'SKILL.md')),
        legacyInstalled: legacyPaths.length > 0,
        legacyPaths,
      }];
    }));
    const result = {
